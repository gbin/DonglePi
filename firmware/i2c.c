...
static bool handle_i2c_write_data_cb(pb_istream_t *stream,
  ...
  l("Length %d", len);
  uint8_t buf[255];
  if (len > sizeof(buf) - 1 || !pb_read(stream, buf, len)) return false;

  l("Data %02x %02x", buf[0], buf[1]);
  struct i2c_master_packet packet = {
      .address = write->addr, .data_length = len, .data = buf,
  };
