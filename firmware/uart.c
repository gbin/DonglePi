...
{ 
	if (udi_cdc_is_rx_ready()) {
		...
		// gbin: usart_write_buffer_job(&usart_module_edbg, &tx_data, 1);
	} else {
		// gbin: usart_disable_callback(&usart_module_edbg, USART_CALLBACK_BUFFER_TRANSMITTED);
		ui_com_rx_stop();
	}
}*/
...
static void usart_rx_callback(const struct usart_module *const module)
{
	...
	// gbin: usart_read_buffer_job(&usart_module_edbg, &rx_data, 1);
...
void uart_rx_notify(uint8_t port)
{
	UNUSED(port);
	// ui_com_rx_start();
	// gbin: usart_enable_callback(&usart_module_edbg, USART_CALLBACK_BUFFER_TRANSMITTED);
	// tx_data = udi_cdc_getc();
	// gbin: usart_write_buffer_job(&usart_module_edbg, &tx_data, 1);
}
...
void uart_open(uint8_t port)
{
	...
	//gbin: usart_disable(&usart_module_edbg);
	//gbin: usart_init(&usart_module_edbg, CONF_USART_BASE, &usart_conf);
	//gbin: usart_enable(&usart_module_edbg);
